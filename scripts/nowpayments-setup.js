...
(async () => {
  console.log('');
  console.log('=== NOWPayments API Setup ===');
  console.log('Your keys will be saved to .env.local (gitignored).');
  console.log('Get your keys from: https://app.nowpayments.io/');
  console.log('  - API Key: Settings → API Keys');
  console.log('  - IPN Secret: Settings → IPN (shown only once on creation)');
  console.log('');

  const apiKey = await ask('Paste your NOWPayments API Key (or press Enter to skip): ');
  const ipnSecret = await ask('Paste your NOWPayments IPN Secret (or press Enter to skip): ');
  ...
  upsert('NOWPAYMENTS_API_KEY', apiKey);
  upsert('NOWPAYMENTS_IPN_SECRET', ipnSecret);
  ...
  console.log(`  NOWPAYMENTS_API_KEY:    ${apiKey ? 'SET (' + apiKey.slice(0, 8) + '...)' : 'unchanged'}`);
  console.log(`  NOWPAYMENTS_IPN_SECRET: ${ipnSecret ? 'SET' : 'unchanged'}`);
