...
export default function PastorPage() {
  ...
  if (!data) return <div className="flex min-h-screen items-center justify-center bg-[var(--bg)]"><div className="h-10 w-10 animate-spin rounded-full border-2 border-[var(--gold)] border-t-transparent" /></div>;
          ...
          {activeTab === "overview" && (
            <div className="space-y-6">
              <div className="grid gap-4 md:grid-cols-4">
                ...
                <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-soft)] p-6">
                  <p className="text-sm text-[var(--muted)]">Available for Payout</p>
                ...
                <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-soft)] p-6">
                  <p className="text-sm text-[var(--muted)]">Share Rate</p>
                  <p className="mt-2 text-3xl font-extrabold text-[var(--cyan)] tabular-nums">{data.shareRate ?? 5}%</p>
                </div>
              </div>
              ...
              <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-soft)] p-6">
                <h3 className="text-lg font-bold text-[var(--fg)]">Flock Summary</h3>
                <div className="mt-4 grid gap-4 md:grid-cols-3">
                  <div className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-4">
                    <p className="text-xs text-[var(--muted)]">Total Flock Deposits</p>
                    <p className="mt-1 text-2xl font-extrabold text-[var(--fg)]">${fmt(flockDeposits)}</p>
                  </div>
                  <div className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-4">
                    <p className="text-xs text-[var(--muted)]">Active Members</p>
                    <p className="mt-1 text-2xl font-extrabold text-[var(--profit)]">{(data.referrals ?? []).filter((r: any) => r.deposited > 0).length}</p>
                  </div>
                  <div className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-4">
                    <p className="text-xs text-[var(--muted)]">Pending Members</p>
                    <p className="mt-1 text-2xl font-extrabold text-[var(--muted)]">{(data.referrals ?? []).filter((r: any) => r.deposited === 0).length}</p>
                  </div>
                </div>
              </div>
          ...
          {activeTab === "earnings" && (
            <div className="space-y-6">
              <div className="grid gap-6 lg:grid-cols-2">
                <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-soft)] p-6">
                  <h3 className="text-lg font-bold text-[var(--fg)]">Request Payout</h3>
                  <p className="mt-1 text-sm text-[var(--muted)]">Available: <b className="text-[var(--gold)]">${fmt(data.available ?? 0)}</b></p>
                  <div className="mt-4 flex gap-3">
                    ...
                    <button onClick={requestPayout} disabled={!payoutAmount || Number(payoutAmount) <= 0 || (data.available ?? 0) <= 0}
                      className="rounded-lg bg-[var(--profit)] px-6 py-3 text-sm font-bold text-[var(--fg)] transition hover:bg-[var(--profit)] disabled:opacity-50">
                      Request
                    </button>
                  </div>
                ...
                <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-soft)] p-6">
                  <h3 className="mb-4 text-lg font-bold text-[var(--fg)]">Payout History</h3>
                  {(data.payouts ?? []).length === 0 ? (
                    <p className="py-6 text-center text-sm text-[var(--muted)]">No payouts yet.</p>
                  ) : (
              ...
              <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-soft)] p-6">
                <h3 className="mb-4 text-lg font-bold text-[var(--fg)]">Daily Earnings</h3>
                {profitHistory.length > 1 ? (
                  <div className="flex items-end gap-1 h-32">
                    {profitHistory.slice(-30).map((v, i) => {
                      const max = Math.max(...profitHistory.slice(-30), 1);
                      const h = (v / max) * 100;
                      return <div key={i} className="flex-1 rounded-t-sm bg-[var(--profit)]/60" style={{ height: `${Math.max(2, h)}%` }} title={`$${v.toFixed(2)}`} />;
                    })}
                  </div>
                ) : (
                  <p className="py-6 text-center text-sm text-[var(--muted)]">Not enough data yet.</p>
                )}
              </div>
            </div>
          )}
          ...
          {activeTab === "engine" && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs text-[var(--muted)]">Watch the engine your flock benefits from</span>
              </div>
              ...
              <div className="space-y-2">
                {(data.leaders ?? []).map((l: any) => (
                  <div key={l.rank} className={`flex items-center gap-4 rounded-xl border px-4 py-3 ${l.isYou ? "border-[var(--gold)] bg-[var(--gold)]/[0.06]" : "border-[var(--border)] bg-[var(--card)]"}`}>
                    <span className={`w-8 text-center text-lg font-extrabold ${l.rank === 1 ? "text-[var(--gold)]" : l.rank === 2 ? "text-[var(--cyan)]" : l.rank === 3 ? "text-purple-400" : "text-[var(--muted)]"}`}>#{l.rank}</span>
